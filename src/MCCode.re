...
let requiredLayers = (density, numBits) => {
    ...
    ceil((sqrt(4.0 *. density ** 2.0 +. 4.0 *. (numBits -. 3.0) *. density +. 9.0) +. 3.0) /. (2.0 *. density))->int_of_float
};
...
let mask = (i, bit) => if (i mod 2 == 0) { bit } else { 1 - bit };

let applyMaskPattern = bits => {
    bits->mapWithIndex(mask)
};

let makeVertices = (n, Config{density, layerWidth, frameWidth, vertexWidthRatio}) => {
    let frameWidthString = to_string(frameWidth);
    ...
    range(0, verticesNum - 1)
        ->map(i => {
            let degree = 360.0 /. float_of_int(verticesNum) *. float_of_int(i);

            <path d={{j|M $sx,$sy A $radius,$radius 0,0,1 $ex,$ey|j}} transform={{j|rotate($degree)|j}} strokeWidth={frameWidthString} fill="none" />
        })->React.array;
};
    ...
    let edges =
        range(0, prevVetricesNum - 1)
            ->map(i => {
                ...
                if (i == 0) {
                    <>
                        <line x1={bx} y1={by} x2={ex1} y2={ey1} transform={{j|translate(0, -$yDelta)|j}} strokeWidth={strokeWidthString} fill="none" />
                        <line x1={bx} y1={by} x2={ex1} y2={ey1} transform={{j|translate(0, $yDelta)|j}} strokeWidth={strokeWidthString} fill="none" />
...
let makeLayer = (n, bits, Config{layerWidth, frameWidth, strokeWidth, vertexWidthRatio} as config) => {
    let strokeWidthString = to_string(strokeWidth);
    ...
    if (n == 0) {
        ...
        <>
            <circle cx="0" cy="0" r={to_string(radius +. frameWidth /. 2.0)} strokeWidth={strokeWidthString} fill="none" />
            <circle cx="0" cy="0" r={to_string(radius -. frameWidth /. 2.0)} strokeWidth={strokeWidthString} fill="none" />
